document.addEventListener("DOMContentLoaded", () => {
  ...
  filterButtons.forEach(button => {
    button.addEventListener("click", () => {
      ...
      products.forEach(product => {
        if(product.classList.contains("empty-card")){
          product.style.display = "block"; // Always show Browse All card
        } else {
    ...
    if (cartItemsContainer) {
      ...
      cart.forEach((item, index) => {
        ...
        <div class="d-flex align-items-center">
          <img src="${item.img}" alt="${item.name}" width="90" class="me-3 rounded">
  ...
  document.querySelectorAll(".add-to-cart").forEach((button, index) => {
    button.addEventListener("click", function () {
      ...
      const price = parseInt((this.dataset.price || "0").replace(/[^\d]/g, ""), 10);
      const oldPrice = parseInt((this.dataset.oldprice || "").replace(/[^\d]/g, ""), 10) || price;
      ...
      [...document.querySelectorAll('[data-bs-toggle="tooltip"]')].forEach(
        (el) => new bootstrap.Tooltip(el, { customClass: "custom-tooltip" })
      );
